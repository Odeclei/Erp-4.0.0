...
class Stops(models.Model):  # Apontamento de Paradas
    ...
    motive = models.ForeignKey(
        StopsMotive, on_delete=models.SET_NULL,
        null=True)
...
class MaquinaBase(models.Model):
    ...
    produto_pai = models.ForeignKey(
        ItemProgramacao, on_delete=models.RESTRICT)
    produto_apontado = models.ForeignKey(
        SubItemProgramacao, on_delete=models.RESTRICT
    )
    ...
    status = models.CharField(max_length=20, choices=[(
        'aberto', 'Aberto'), ('concluido', 'Concluído')], default='aberto')
