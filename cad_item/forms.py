...
class ItemForm(forms.ModelForm):
    ...
    class Meta:
        model = Item
        fields = (
            "item_cod", 'name_prod', 'name_abrev', 'semi_code',
            'qtd_per_day', 'is_active', 'family',
        )
        widgets = {
            'family': forms.Select(
                attrs={
        ...
        help_texts = {
            'item_cod': 'Formato código = "000.0000.00.00"'
        }
...
class SubItemForm(forms.ModelForm):
    class Meta:
        model = SubItem
        fields = (
            'subitem_cod', 'name_subitem', 'observation', 'ficha_tecnica',
            'variation', 'is_active', 'material', 'comprimento', 'largura', 'espessura',
        )
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['name_subitem'].widget.attrs.update(
            {'class': 'form-control'})
        self.fields['largura'].widget.attrs.update({'class': 'form-control'})
        self.fields['comprimento'].widget.attrs.update(
            {'class': 'form-control'})
...
class EstruturaForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'subitem': forms.Select(attrs={'class': 'form-control form-control-lg select2'}),
            'qntde_pre': forms.NumberInput(attrs={'class': 'form-control form-control-lg'}),
            'qntde_usi': forms.NumberInput(attrs={'class': 'form-control form-control-lg'}),
            'qntde_lix': forms.NumberInput(attrs={'class': 'form-control form-control-lg'}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'subitem' in self.data:
            self.fields['subitem'].\
                queryset = SubItem.objects.all()  # type: ignore
