...
class Item(models.Model):
    ...
    name_prod = models.CharField(
        max_length=100, verbose_name='Nome Produto')
    name_abrev = models.CharField(
        max_length=50, verbose_name='Nome Resumido')
    semi_code = models.CharField(
        max_length=50, verbose_name='Referência Semiacabado',
        help_text='Digitar código sem ponto "."')
    qtd_per_day = models.IntegerField(
        verbose_name='Peças ao dia')
    is_active = models.BooleanField(
        default=True, verbose_name='Ativo')
    family = models.ForeignKey(
        FamilyProd, on_delete=models.SET_NULL,
        null=True, blank=True, verbose_name='Família Produto')
    ...
    def obter_subprodutos_e_quantidades(self):
        ...
        subprodutos_quantidade = [{
            ...
            'quantidade': estrutura.qntde_pre} for estrutura in subprodutos]
...
class SubItem(models.Model):
    ...
    subitem_cod = models.CharField(
        max_length=20, unique=True,
        verbose_name='Código')
    name_subitem = models.CharField(max_length=100,
                                    verbose_name='Nome Sub-Item')
    ...
    ficha_tecnica = models.ImageField(
        upload_to='assets/ficha_tecnica/%Y%m/',
        default='', blank=True)
    variation = models.CharField(
        max_length=50, null=True, blank=True, verbose_name='Variação')
    material = models.ForeignKey("Material", verbose_name=(
...
class Estrutura(models.Model):
    ...
    subitem = models.ForeignKey(
        SubItem, on_delete=models.SET_NULL,
        null=True, blank=True)
