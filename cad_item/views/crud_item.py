...
class ItemCreateView(CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy('cad_item:detail',
...
class ItemUpdateView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        form_action = reverse_lazy(
            'cad_item:update', kwargs={'pk': self.object.pk}
        )
        family_list = FamilyProd.objects.all().order_by('description')
        context.update({
            'form_action': form_action,
    ...
    def get(self, request, *args, **kwargs):
        ...
        if termo_busca:
            families = FamilyProd.objects.filter(
                ...
                Q(refer__icontains=termo_busca)
            ).order_by('refer')[:10]

            for family in families:
                family_list.append({
                    'id': family.pk,
                    'text': f"{family.description}"
                })
...
class ItemDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        create_estrtura_url = reverse_lazy(
            'cad_item:estrutura', args=[self.object.pk])  # type: ignore

        estrutura_list = Estrutura.objects.all().\
...
class ItemEstruturaCreateView(CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy('cad_item:estrutura',
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        pk_produto = self.kwargs["pk"]
        form_action = reverse_lazy('cad_item:estrutura', kwargs={
            'pk': pk_produto})  # type: ignore
