...
class OrderDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ajax_url = reverse_lazy('order:add_item',
                                kwargs={'pk': self.kwargs['pk']})
        pk_order = self.kwargs['pk']

        itens_list = ItemProgramacao.objects.all().\
...
class OrderSearchView(ListView):
    ...
    def get_queryset(self):
        ...
        return super().get_queryset().filter(
            order_number__icontains=search_value
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if termo_busca:
            itens_query = Item.objects.filter(
                Q(item_cod__icontains=termo_busca) |
                Q(name_prod__icontains=termo_busca)
            ).order_by('item_cod')[:10]

            for item in itens_query:
                itens_list.append({
                    'id': item.pk,
                    'text': f'{item.item_cod} - {item.name_prod}'})
